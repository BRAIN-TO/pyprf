...
#############################################################################################
##COMMENTED OUT AS MODIFIED TO CHANGE THE FIXATION CIRCLE TO A CROSS#########################
#    # Fixation dot:
...
#        interpolate=False,
#        autoLog=False)
    ## ADD FIXATION CROSS GENERATION########################################################
    objFix = visual.TextStim(
        objWin,
        text='+',
        pos=(0.0, 0.0),
        height=1.0,        # size in your window units
        color=lstClrFix,
        colorSpace='rgb')
    
    objFixSrd = visual.TextStim(
        objWin,
        text='+',
        pos=(0.0, 0.0),
        height=1.0,        # size in your window units
...
#            barColours = np.random.randint(0, 256, size=3).astype(np.uint8)
#            barColours = np.float32(barColours / 127.5) - 1.0
#
#            objBar.setColor((barColours[0], barColours[1], barColours[2]))
...
#############################################################################################
                ...
                objAprtr = visual.Aperture(
                    objWin,
                    size=varDegCov,
                    shape=vertices,    # select draw position based on X position
                    pos=(0, 0),
                    inverted=False,
                    units='deg')

                # Enable the aperture before drawing
                objAprtr.enabled = True
#############################################################################################
                # Draw the stimulus bar
                objBar.draw(win=objWin)
#############################################################################################
                # Outside the 'with' block, drawing returns to normal (no clipping)
...
#############################################################################################
            ...
            if ((varTmeTrgt <= varTme02)
                ...
                if varSwtTrgt == 0:
                    ...
                    objFix.color = lstClrTrgt #Change for object type
                ...
                if varSwtTrgt == 1:
                    ...
                    # back to normal).
                    objFix.color = lstClrFix #Change for object type
...
if __name__ == "__main__":
    ...
    dicParam = {'Run (name of design matrix file)': 'Hemianopsia_Run_01',
                ...
                'Distance between observer and monitor [cm]': 233.0,
                'Width of monitor [cm]': 70.0,
    ...
    if objGui.OK is True:
        ...
        strPthOut = os.path.join(strPth,
                                 ...
                                 (dicParam['Run (name of design matrix file)']
                                  + '_alternating_'# CHANGE LOG FILE NAME
                                  + strDate
                                  + '.txt')
                                 )
