...
#
# 20260306 by Darren Liang, for BRAIN-TO Lab, University of Toronto
# Modifications made to lines 688-710 to present bar stimuli on the right half of 
# the screen only, for a retinotopic mapping experiment with non-square visual 
# field coverage. The bar stimulus is drawn within an aperture that is defined by the 
# right half of the screen. The aperture is enabled before drawing the stimulus bar, 
# and disabled afterwards to return to normal drawing (no clipping).
...
#############################################################################################
                right_half_vertices = [(0, -1), (1, -1), (1, 1), (0, 1)]
                left_half_vertices = [(-1, -1), (0, -1), (0, 1), (-1, 1)]
                ...
                objAprtr = visual.Aperture(
                    objWin,
                    size=varDegCov,
                    shape=right_half_vertices,    # right half screen draw only for the grating
                    pos=(0, 0),
                    inverted=False,
                    units='deg')
